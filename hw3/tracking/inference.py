...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None :
            ...
            for pos in self.beliefs.sortedKeys() :
                dis = util.manhattanDistance(pos, pacmanPosition)
                if emissionModel[dis] > 0 :
                    allPossible[pos] = self.beliefs[pos] * emissionModel[dis]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.beliefs.sortedKeys() :
            for nextPos, prob in self.getPositionDistribution(self.setGhostPosition(gameState, pos)).items() :
                if nextPos not in newBeliefs.sortedKeys() : newBeliefs[nextPos] = 0
                newBeliefs[nextPos] += prob * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        i, period = 0, len(self.legalPositions)
        particles = []
        while i < self.numParticles :
            particles.append(self.legalPositions[i % period])
            i += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None :
            ...
            for pos in self.particles :
                if pos not in weightedPos.keys() : weightedPos[pos] = 0
                dis = util.manhattanDistance(pacmanPosition, pos)
                weightedPos[pos] += emissionModel[dis]
            weightedPosValues = [pair[0] for pair in list(weightedPos.iteritems())]
            weightedPosDistr = [pair[1] for pair in list(weightedPos.iteritems())]
            if sum(weightedPosDistr) == 0 :
                self.initializeUniformly(gameState)
            else :
                samples = util.nSample(weightedPosDistr, weightedPosValues, self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles :
            for pos, prob in self.getPositionDistribution(self.setGhostPosition(gameState, particle)).items() :
                if pos not in weightedPos.keys() : weightedPos[pos] = 0
                weightedPos[pos] += prob
        weightedPosValues, weightedPosDistr = [], []
        for value, distr in weightedPos.iteritems() :
            weightedPosValues.append(value)
            weightedPosDistr.append(distr)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles :
            if particle not in beliefs : beliefs[particle] = 0
            beliefs[particle] += 1
        beliefs.normalize()
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while len(particles) < self.numParticles :
            particles = particles + list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        particles = particles[0:self.numParticles]
        ...
        for particle in self.particles :
            ...
            for i, pos in enumerate(particle) :
                if noisyDistances[i] != None :
                    ...
                    particleTemp = list(particle)
                    particleTemp[i] = self.getJailPosition(i)
                    particle = tuple(particleTemp)
            if particle not in weightedPos.keys() : weightedPos[particle] = 0
            weightedPos[particle] += weight
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i, pos in enumerate(newParticle) :
                ps = []
                probs = []
                for p, prob in getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i]).iteritems() :
                    ps.append(p)
                    probs.append(prob)
                sample = util.sample(probs, ps)
                newParticleTemp.append(sample)
            newParticle = newParticleTemp
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        beliefs = util.Counter()
        for particle in self.particles :
            if particle not in beliefs.keys() : beliefs[particle] = 0
            beliefs[particle] += 1
        beliefs.normalize()
        return (beliefs)
